...
import me.VideoSRC.tempos.Cooldown;
...
public class Grappler implements Listener {
	...
	public void onLeash(PlayerLeashEntityEvent e) {
		if (Habilidade.getAbility(e.getPlayer()) == "grappler") {
	 		e.setCancelled(true);
    ...
    public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) {
        ...
        if (event.getEntity() instanceof Player && event.getDamager() instanceof Player) {
            ...
            if (Habilidade.getAbility(player) == "grappler") {
            	Cooldown.add(player, 4);
            }
        }
    }
	...
	public void usar(PlayerInteractEvent e) {
		Player p = e.getPlayer();
	 	if ((p.getItemInHand().getType().equals(Material.LEASH)) && Habilidade.getAbility(p) == "grappler") {
	 	     e.setCancelled(true);
			 if (Cooldown.add(p)) {
