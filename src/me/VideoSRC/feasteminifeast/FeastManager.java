...
public class FeastManager {
	...
	public static void announceFeast(Integer time) {
		...
		Bukkit.broadcastMessage("§cFEAST§7, O Feast nascera em: §f" + df.format(mainBlock.getLocation().getX())
				+ "‚§7,‚§f " + df.format(mainBlock.getLocation().getY()) + "‚§7,‚§f "
				+ df.format(mainBlock.getLocation().getZ()) + "‚§7 em ‚§f" + time + " ‚§7minuto" + s
				+ "‚§7 Use ‚§f/Feast");
		...
		for (Object item : items) {
			...
			if (spawn.booleanValue()) {
				...
				if (maxamount == minamount) {
					...
					amount = Integer
							.valueOf(minamount.intValue() + r.nextInt(maxamount.intValue() - minamount.intValue() + 1));
				}
				int maxtry;
				if (Main.BausFeast.booleanValue()) {
					for (; amount.intValue() > 0; amount = Integer.valueOf(amount.intValue() - 1)) {
				...
				c.add(-(ra.intValue() / 2) + r.nextInt(ra.intValue()), 1.0D,
						-(ra.intValue() / 2) + r.nextInt(ra.intValue()));
